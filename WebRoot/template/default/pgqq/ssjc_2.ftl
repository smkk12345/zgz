...
<div class="ssjc_2 container-fluid  clearfix">
    <#if chartXAxis??>
    ...
    <div  class="menu mt20 clearfix pull-right ">
        ...
        <div class="form-group clearfix pull-right ml5">
            <select class="form-control width-80px pull-right bold" id="WSS-Select">
                <option selected='selected' value="${BASE_PATH}lqjc/ssjc_2.action">温度</option>
                <option value="${BASE_PATH}lqjc/ssjc_3.action">湿度</option>
                <option value="${BASE_PATH}lqjc/ssjc_4.action">水分</option>
            </select>
        </div>
    </div>
    ...
    <div  class="menu mt10 clearfix fl">
        <div class="form-group fl yu-switch">
            <ul class="mt0 clearfix bootstrap-switch-mini">
                ...
                <li class="base-sline">粮堆均温：<input  type="checkbox" name="switch" value="B1" checked="checked"></li>
                <li class=" base-sline">仓温：<input  type="checkbox" name="switch" value="B2" checked="checked"></li>
                <li class="base-sline">外温：<input  type="checkbox" name="switch" value="B3" checked="checked"></li>
    ...
    <script type="text/javascript">
                ...
                $("input[name='switch-all']").on('switchChange.bootstrapSwitch', function (e, state) {
                    ...
                    if ($("input[name='switch-all']").is(':checked')) {
                        $("input[name='switch']").each(function () {
                            if ($(this).val() !== "B1" && $(this).val() !== "B2" && $(this).val() !== "B3") {
                ...
                function getChartData(ids, startDateStr, endDate, grainId) {
                    var url = '${BASE_PATH}lqjc/getChartData.action?oType=' + "wd";
                    $.ajax({
                        type: "POST",
                        url: url,
                        dataType: "json",
                        data: {ids: ids, startDateStr: startDateStr, endDateStr: endDate, grainId: grainId, oType: "wd"},
                        ...
                        success: function (response) {
                            ...
                            if (curChartXAxis != null) {
                                CallChart('', curChartXAxis, curChartSeries, "平均温度曲线分析图", "（最近监测时间：${CommenDate.RecentDate}）", "温度(℃)");
                            } else {
                                alert("此时间段没有粮情数据！");
                            }
                        }
                 ...
                 */
                function Chart() {
                    CallChart('', '${chartXAxis}', '${chartSeries}', "平均温度曲线分析图", "（最近监测时间：${CommenDate.RecentDate}）", "温度(℃)");
                }
                //选择框切换温湿水
