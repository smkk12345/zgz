...
<div class="container-fluid ssjc_1">
    ...
    <table id='yu-print-show' class="table table-condensed table-bordered mt10">
        <tr>
            <th  colspan="${arrangementBean.columnCount?number*4+2}" class="danger textalign-vc" style="line-height: 29px">
        ...
        <tr class="warning">
            ...
            <th colspan="${arrangementBean.columnCount?number*4-4}" class="textalign-l"></th>
        </tr>
        <tr class="warning">
            <th colspan="4" class="textalign-l">实际储存量：${CommenData.cr}吨</th>
            <th colspan="5" class="textalign-l">粮食品种：${CommenData.lspz}</th>
            <th colspan="7" class="textalign-l">入仓时间：${CommenData.time_rc}</th>
            <th colspan="${arrangementBean.columnCount?number*4-11}" class="textalign-l">等级：${CommenData.grade}</th>
        </tr>
        <tr class="warning">
            <th colspan="4" class="textalign-l">收获年份：${CommenData.time_sh}</th>
            <th colspan="5" class="textalign-l">入仓水分：${CommenData.moisture}%</th>
            <th colspan="${arrangementBean.columnCount?number*4-7}" class="textalign-l">保管单位：${CommenData.bgdw}</th>
        </tr>
        <tr  class="warning">
            <th colspan="${arrangementBean.columnCount?number*4+2}" class="textalign-l">说明：默认显示每个点的温度，鼠标放上去显示湿度、水分、露点等参数</th>
        </tr>
        <tr  class="success">
        	<td colspan="2" rowspan="2">层/行/列</td>
            <#list 1..arrangementBean.columnCount?number as x>
            <td colspan = 4>${x_index+1}列</td>
            </#list>
        </tr>
        <tr  class="success">
            <#list 1..arrangementBean.columnCount?number as x>
          ...
          <tr  class="warning">
            <th colspan="${arrangementBean.columnCount?number*4+2}" class="textalign-l"></th>
             </tr>       
        <#list lqData as vo>
	        <#list vo as vo1>
	        <tr  class="active">
	            <#if vo1_index ==0>
	            	<td rowspan="${arrangementBean.rowCount}" class="textalign-vc">${vo_index+1}层</td>
	            </#if>
	            <td>${vo1_index+1}行</td>
	            <#list vo1 as vo2>
	            <#if vo2.wd?string !="">
	            <td style="background:#C6E3CC" wd='<#if vo2.wd?string !="">${vo2.wd}℃</#if>' sd='<#if vo2.sd?string !="">${vo2.sd}%</#if>' sf='<#if vo2.sf?string !="">${vo2.sf}%</#if>' ld='<#if vo2.ld?string !="">${vo2.ld}%</#if>'  c='${vo1_index+1}' h='${vo_index+1}' l='${vo2_index+1}'  class="lqsj" onMouseOver="javascript:show(this, 'show-point-data');" onMouseOut="hide(this, 'show-point-data')">
