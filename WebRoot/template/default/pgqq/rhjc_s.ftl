...
<div class="container-fluid rhjc">
    <ul class="nav nav-pills menu mb5 clearfix">
        <div class="page-title">
            <h4 class="bold">入户基础信息审计审核</h4>
        </div>
        <div class="top-ation mb10 clearfix">
            <div class="fl">
                <form  action="${BASE_PATH}pgqq/rhjc_s.action" accept-charset="UTF-8" method="post" >
                    ...
                    <div class='autoWidthGroup-2 fl'>
                     ...
                     <div class='autoWidthGroup-2 fl'>
                        <label class='fl control-label'>审核状态:</label>
                         <select id='sexsel' class='form-control input-sm fl' style='width:120px;' name='checkresult' >  
	                        <option  <#if (checkresult?default("-1"))=='-1'>selected='selected'</#if> value="-1">请选择</option>  
	                        <option  <#if (checkresult?default("-1"))=='0'>selected='selected'</#if>  value="0">未审核通过</option>  
	                        <option  <#if (checkresult?default("-1"))=='1'>selected='selected'</#if> value="1">审核通过</option> 
                    	</select>
                    </div>    
        ...
        <div id="yu-pager" class="fl mb20">
            ...
            <#if recordCount??>
            <@p.pager pageNo=pageNo pageSize=pageSize recordCount=recordCount toURL="/pgqq/rhjc_s.action" OtherParameter=""/>
            </#if>
        </div>
    </ul>
</div>
