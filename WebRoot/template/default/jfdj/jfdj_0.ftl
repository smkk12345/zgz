...
<div class="container-fluid rhjc">
    <div class="page-title">
        <ul class="nav nav-pills menu mb10">
            <li role="presentation" ><a href="${BASE_PATH}jfdj/jfdj_1.action">已交房</a></li>
            <li role="presentation" class="active"><a href="${BASE_PATH}jfdj/jfdj_0.action">未交房</a></li>
        </ul>
    </div>
    ...
    <div id="yu-pager" class="fl mb20">
        ...
        <#if recordCount??>
        <@p.pager pageNo=pageNo pageSize=pageSize recordCount=recordCount toURL="/jfdj_0.action" OtherParameter=""/>
        </#if>
    </div>
</ul>
</div>
    ...
    function confirmOthers(btn) {
        if (yu_confirm("您确认已经交房？")) {
            ...
            $.ajax({
                ...
                dataType: "json",
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    location.href = "${BASE_PATH}" + "jfdj_0.action";
                },
                success: function (response) {
                    location.href = "${BASE_PATH}" + "jfdj_0.action";
                }
