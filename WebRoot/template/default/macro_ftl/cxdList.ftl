...
<tr id="${ROW.id}"  data-url="${BASE_PATH}user/yhglModal.action?userId=${ROW.id}">
    ...
    <td>    
        ...
        <button type="button" class="btn btn-info btn-xs ml10 YL-ModalClick glyphicon glyphicon-print"  title='打印' data-url="${BASE_PATH}pgqq/rhjc_print.action?housebasicid=${ROW.id}"> </button>
    </td>
</tr>
</#list>
</#if>
...
<tr id="${ROW.id}">
    ...
    <td>
     <#if role.roleAuthority[13]=="2">
        <button type="button" class="btn btn-danger btn-xs ml10 YL-ModalClick bold glyphicon glyphicon-print" title='打印告知单' data-url="${BASE_PATH}pgzq/fhxy_sxh_print_Modal.action?housebasicid=${ROW.housebasicid}&agreenmentid=${(ROW.agreenmentid)!""}"></button>
    </#if>
    </td>
